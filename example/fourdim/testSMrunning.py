...
# Higgs mass parameter: V ⊃ - m²(H†H), so mh² = 2m² at tree level
m2_0 = mh**2 / 2
...
print(f"  m²     = {m2_0:.2f} GeV²")

rge = SMRGE(MZ, g_Y=gY0, g2=g20, g3=g30, yt=yt0, yb=yb0, ytau=ytau0, lam=lam0, m2=m2_0)
...
m2_run   = [rge.get_m2(mu)   for mu in mu_values]
...
ax = axes[1, 1]
ax.plot(np.log10(mu_values), m2_run, color='C4', label=r'$m^2$')
ax.axhline(0, color='k', lw=0.8, ls='--')
ax.set_xlabel(r'$\log_{10}(\mu/\mathrm{GeV})$')
ax.set_ylabel(r'$m^2\,[\mathrm{GeV}^2]$')
ax.set_title('Higgs mass parameter')
ax.legend()
ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig("SMrunning.pdf")
